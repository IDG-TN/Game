<template>
  <div class="game-carrier">
    ...
    <iframe id="some_frame_id" :src="src" scrolling="no"></iframe>
  </div>
</template>
...
export default {
  created() {
    if (this.$route.query.gameUrl != undefined) {
      this.src = JSON.parse(this.$route.query.gameUrl);
    }
  },
  data() {
    return {
  ...
  mounted() {
    let hashH = document.documentElement.scrollHeight; //获取自身高
    this.queryApi("#some_frame_id").style.height = hashH / 100 + "rem";
    this.queryApi(".game-carrier").style.height = hashH / 100 + "rem";
    this.queryApi(".game-carrier").style.overflow = "hidden";
  },
  methods: {
    popupAddevenet() {
